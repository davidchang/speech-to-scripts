...
/***/ function(module, exports, __webpack_require__) {
	...
	var recognitionSucceeded = function(e ) {
	  ...
	  getCommands().then(function(commands ) {
	    ...
	    _.each(pertinentCommands, function(command ) {
	      var regex = new RegExp(command.keywords);
	      console.log('command.script', command.script);
	      (function(captured ) {return eval(command.script)})(regex.exec(result).splice(1));
	    });
	...
	chrome.runtime.onInstalled.addListener(function(details ) {
	  ...
	  if (_.contains(['install', 'update'], details.reason)) {
	    chrome.storage.sync.get('speechEnabled', function(value ) {
